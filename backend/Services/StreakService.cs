...
namespace BookTrackingSystem.Services
{
    public class StreakService : IStreakService
    {
        ...
        public async Task<Result<StreakDto>> GetStreakDataAsync(DateTime? clientLocalToday = null)
        {
            try
            {
                ...
                return Result<StreakDto>.Success(data ?? new StreakDto());
            }
            catch (Exception ex)
            {
                return Result<StreakDto>.Failure($"An error occurred while retrieving streak data: {ex.Message}");
            }
        }
    }
}