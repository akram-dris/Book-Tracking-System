...
namespace BookTrackingSystem.Services
{
    public class StatisticsService : IStatisticsService
    {
        ...
        public async Task<Result<ReadingOverviewDto>> GetReadingOverviewAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                ...
                var sessions = await _context.ReadingSessions
                    ...
                    .ToListAsync();
                var streakResult = await _streakService.GetStreakDataAsync();
                var streakData = streakResult.IsSuccess ? streakResult.Data! : new StreakDto();
                ...
                var dto = new ReadingOverviewDto
                {
                ...
                return Result<ReadingOverviewDto>.Success(dto);
            }
            catch (Exception ex)
            {
                return Result<ReadingOverviewDto>.Failure($"An error occurred while retrieving reading overview: {ex.Message}");
            }
        }

        public async Task<Result<AuthorStatisticsDto>> GetAuthorStatisticsAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
            ...
            {
                return Result<AuthorStatisticsDto>.Failure($"An error occurred while retrieving author statistics: {ex.Message}");
            }
        }

        public async Task<Result<TagStatisticsDto>> GetTagStatisticsAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
                ...
                return Result<TagStatisticsDto>.Success(dto);
            }
            catch (Exception ex)
            {
                return Result<TagStatisticsDto>.Failure($"An error occurred while retrieving tag statistics: {ex.Message}");
            }
        }

        public async Task<Result<TimeBasedStatisticsDto>> GetTimeBasedStatisticsAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
                ...
                var dto = new TimeBasedStatisticsDto
                {
                ...
                return Result<TimeBasedStatisticsDto>.Success(dto);
            }
            catch (Exception ex)
            {
                return Result<TimeBasedStatisticsDto>.Failure($"An error occurred while retrieving time-based statistics: {ex.Message}");
            }
        }

        public async Task<Result<GoalPerformanceDto>> GetGoalPerformanceAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
                ...
                var dto = new GoalPerformanceDto
                {
                ...
                return Result<GoalPerformanceDto>.Success(dto);
            }
            catch (Exception ex)
            {
                return Result<GoalPerformanceDto>.Failure($"An error occurred while retrieving goal performance: {ex.Message}");
            }
        }

        public async Task<Result<BookStatisticsDto>> GetBookStatisticsAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
            ...
            {
                return Result<BookStatisticsDto>.Failure($"An error occurred while retrieving book statistics: {ex.Message}");
            }
        }

        public async Task<Result<PersonalRecordsDto>> GetPersonalRecordsAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
                ...
                var dto = new PersonalRecordsDto
                {
                ...
                return Result<PersonalRecordsDto>.Success(dto);
            }
            catch (Exception ex)
            {
                return Result<PersonalRecordsDto>.Failure($"An error occurred while retrieving personal records: {ex.Message}");
            }
        }

        public async Task<Result<StatisticsDto>> GetCompleteStatisticsAsync(StatisticsFilterDto? filter = null)
        {
            try
            {
                filter ??= new StatisticsFilterDto { FilterType = FilterType.Year };
                
                var overviewResult = await GetReadingOverviewAsync(filter);
                var authorsResult = await GetAuthorStatisticsAsync(filter);
                var tagsResult = await GetTagStatisticsAsync(filter);
                var timeBasedResult = await GetTimeBasedStatisticsAsync(filter);
                var goalsResult = await GetGoalPerformanceAsync(filter);
                var booksResult = await GetBookStatisticsAsync(filter);
                var recordsResult = await GetPersonalRecordsAsync(filter);

                if (!overviewResult.IsSuccess) return Result<StatisticsDto>.Failure(overviewResult.Errors);
                if (!authorsResult.IsSuccess) return Result<StatisticsDto>.Failure(authorsResult.Errors);
                if (!tagsResult.IsSuccess) return Result<StatisticsDto>.Failure(tagsResult.Errors);
                if (!timeBasedResult.IsSuccess) return Result<StatisticsDto>.Failure(timeBasedResult.Errors);
                if (!goalsResult.IsSuccess) return Result<StatisticsDto>.Failure(goalsResult.Errors);
                if (!booksResult.IsSuccess) return Result<StatisticsDto>.Failure(booksResult.Errors);
                if (!recordsResult.IsSuccess) return Result<StatisticsDto>.Failure(recordsResult.Errors);
                ...
                var dto = new StatisticsDto
                {
                    ...
                    TimeBased = timeBasedResult.Data!,
                    Goals = goalsResult.Data!,
                    Books = booksResult.Data!,
                    Records = recordsResult.Data!
                };
                return Result<StatisticsDto>.Success(dto);
            }
            catch (Exception ex)
            {
                return Result<StatisticsDto>.Failure($"An error occurred while retrieving complete statistics: {ex.Message}");
            }
        }
